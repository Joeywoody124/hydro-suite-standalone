...
Change History:
    ...
    v2.0 - Full GUI integration, split HSG handling
    v2.1 - Lookup table loading improvements
    v2.2 - Output formatting enhancements
    v2.3 - Area recalculation fix after intersections, CN integer output,
           geometry repair steps, area validation logging
"""
...
from qgis.core import (
    ...
    QgsWkbTypes, Qgis, QgsMessageLog, QgsExpression, QgsExpressionContext,
...
class CNCalculatorTool(HydroToolInterface, LayerSelectionMixin):
    ...
    def run(self, progress_callback: Optional[Callable[[int, str], None]] = None) -> bool:
        ...
        try:
            ...
            progress_callback(5, "Validating layer geometry and CRS...")

            # Create feedback object for QGIS processing
            feedback = QgsProcessingFeedback()

            # Reproject layers to target CRS if needed
            progress_callback(10, "Reprojecting layers...")
            ...
            # Fix geometries before intersection to prevent topology issues
            progress_callback(20, "Repairing input geometries...")
            subbasin_fixed = self.fix_geometries(subbasin_reproj, feedback)
            landuse_fixed = self.fix_geometries(landuse_reproj, feedback)
            soils_fixed = self.fix_geometries(soils_reproj, feedback)

            # Log reference area from subbasins (dissolved total)
            ref_area_acres = self._calculate_total_layer_area(subbasin_fixed)
            self.progress_logger.log(
                f"Reference subbasin total area: {ref_area_acres:.3f} acres"
            )

            # First intersection: land use with soils
            progress_callback(35, "Intersecting land use with soils...")
            lu_soil = self.intersect_layers(landuse_fixed, soils_fixed, feedback)

            # Fix geometries after first intersection
            progress_callback(45, "Repairing intersection geometries...")
            lu_soil_fixed = self.fix_geometries(lu_soil, feedback)

            # Second intersection: subbasins with land use/soils
            progress_callback(55, "Intersecting subbasins with land use/soils...")
            final_intersection = self.intersect_layers(subbasin_fixed, lu_soil_fixed, feedback)

            # Fix geometries after second intersection and recalculate areas
            progress_callback(65, "Repairing final intersection and recalculating areas...")
            final_fixed = self.fix_geometries(final_intersection, feedback)

            # Log post-intersection area for validation
            post_area_acres = self._calculate_total_layer_area(final_fixed)
            area_diff = abs(ref_area_acres - post_area_acres)
            area_diff_pct = (area_diff / ref_area_acres * 100) if ref_area_acres > 0 else 0
            self.progress_logger.log(
                f"Post-intersection total area: {post_area_acres:.3f} acres "
                f"(diff: {area_diff:.3f} ac / {area_diff_pct:.2f}%)"
            )
            if area_diff_pct > 1.0:
                self.progress_logger.log(
                    f"⚠️ Area difference exceeds 1% — review input layer topology",
                    "warning"
                )

            # Calculate composite CN values (uses recalculated geometry areas)
            progress_callback(75, "Calculating composite CN values...")
            results = self.calculate_composite_cn(
                final_fixed, subbasin_field, landuse_field, soils_field
            )

            # Store reference area for outputs
            results['reference_area_acres'] = ref_area_acres
            results['intersection_area_acres'] = post_area_acres

            # Create output
            progress_callback(90, "Creating output files...")
            self.create_outputs(subbasin_fixed, results, subbasin_field, output_dir)
    ...
    def reproject_layer(self, layer: QgsVectorLayer, feedback) -> QgsVectorLayer:
        ...
        self.progress_logger.log(
            f"Reprojecting {layer.name()} from {layer.crs().authid()} to {self.target_crs.authid()}"
        )
    ...
    def fix_geometries(self, layer: QgsVectorLayer, feedback) -> QgsVectorLayer:
        """
        Repair geometries to eliminate slivers, self-intersections, and
        topology errors that cause area discrepancies after intersection.
        """
        self.progress_logger.log(f"Fixing geometries for: {layer.name()}")

        params = {
            'INPUT': layer,
            'METHOD': 1,   # Structure method (recommended for polygon operations)
            'OUTPUT': 'memory:'
        }
        result = processing.run("native:fixgeometries", params, feedback=feedback)
        return result['OUTPUT']

    def intersect_layers(self, layer1: QgsVectorLayer, layer2: QgsVectorLayer,
                         feedback) -> QgsVectorLayer:
        """
        Perform intersection between two layers.
        
        Note: After intersection, polygon areas change because features are
        clipped to their mutual overlap. Always recalculate areas from the
        resulting geometries — never carry forward areas from the input layers.
        """
        ...
        if intersection.featureCount() == 0:
            raise ValueError(
                f"Intersection between {layer1.name()} and {layer2.name()} "
                f"resulted in no features"
            )

        self.progress_logger.log(
            f"  Intersection produced {intersection.featureCount()} features"
        )

        return intersection

    def _calculate_total_layer_area(self, layer: QgsVectorLayer) -> float:
        """
        Calculate total area of all features in a layer (in acres).
        Uses geometry().area() which returns area in the layer's CRS units
        (for EPSG:3361 = US Survey Feet, so area is in sq ft).
        """
        total_sqft = 0.0
        for feature in layer.getFeatures():
            geom = feature.geometry()
            if geom and not geom.isNull():
                total_sqft += geom.area()
        return total_sqft / 43560.0

    def calculate_composite_cn(self, intersection_layer: QgsVectorLayer,
                               subbasin_field: str, landuse_field: str,
                               soils_field: str) -> Dict:
        """
        Calculate composite CN values for each subbasin.
        
        Areas are computed directly from the post-intersection geometry,
        ensuring they reflect the actual clipped polygons rather than any
        pre-existing area attributes from the input layers.
        """
        subbasin_data = {}
        detailed_records = []
        skipped_count = 0
        ...
        for feature in intersection_layer.getFeatures():
            ...
            # Calculate area from the actual post-intersection geometry
            geom = feature.geometry()
            ...
            # Skip degenerate slivers (< 1 sq ft ~ 0.00002 acres)
            if area_sqft < 1.0:
                skipped_count += 1
                continue
            ...
            if cn_key not in self.lookup_data:
                self.progress_logger.log(
                    f"Warning: No CN found for land use '{landuse_code}' "
                    f"and soil group '{soil_group}'",
                    "warning"
                )
                continue
        ...
        if skipped_count > 0:
            self.progress_logger.log(
                f"Skipped {skipped_count} degenerate/sliver features (< 1 sq ft)",
                "warning"
            )
    ...
    def parse_soil_group(self, soil_group_raw: str) -> str:
        ...
        if '/' in soil_group:
            ...
            if len(parts) >= 2:
                ...
                self.progress_logger.log(
                    f"Split HSG detected: '{soil_group_raw}' -> using '{soil_group}'"
                )
        ...
        if soil_group in ['A', 'B', 'C', 'D']:
            ...
            self.progress_logger.log(
                f"Warning: Invalid soil group '{soil_group_raw}', using 'd' as default",
                "warning"
            )
    ...
    def create_outputs(self, subbasin_layer: QgsVectorLayer, results: Dict,
        ...
        """Create output files with both decimal and integer CN values"""
        from qgis.PyQt.QtCore import QVariant

        # Create output layer with CN fields
        output_layer = QgsVectorLayer(
            f"Polygon?crs={self.target_crs.authid()}", "subbasins_cn", "memory"
        )
        ...
        # Decimal CN value (area-weighted)
        new_fields.append(QgsField("CN_Comp", QVariant.Double, "double", 10, 2))
        # Integer CN value (rounded)
        new_fields.append(QgsField("CN_Int", QVariant.Int, "integer", 5, 0))
        # Area from intersection calculation
        ...
        for orig_feature in subbasin_layer.getFeatures():
            ...
            # Calculate and add CN_Comp (decimal) and CN_Int (integer)
            ...
            attributes.extend([cn_comp, cn_int, total_area])
        ...
        # Save summary CSV
        self.save_summary_csv(subbasin_data, results, output_dir)

        self.progress_logger.log(f"Outputs saved to {output_dir}")

    def save_detailed_csv(self, detailed_records: list, subbasin_data: dict,
                          output_dir: str):
        """Save detailed calculation CSV with both decimal and integer CN"""
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for subbasin_id in sorted(subbasin_groups.keys()):
                data = subbasin_data[subbasin_id]
                cn_composite = (
                    data['cn_area_sum'] / data['total_area']
                    if data['total_area'] > 0 else 0
                )
                cn_integer = round(cn_composite)

                # Subbasin header row
                writer.writerow([
                    'Subbasin ID', 'Total Area (acres)',
                    'Composite CN (decimal)', 'Composite CN (integer)',
                    '', '', '', ''
                ])
                writer.writerow([
                    subbasin_id,
                    round(data['total_area'], 2),
                    round(cn_composite, 2),
                    cn_integer,
                    '', '', '', ''
                ])
                ...
                # Detail header
                writer.writerow([
                    '', 'Land Use', 'Soil Type', 'Area (acres)',
                    'CN Value', 'CN x Area', 'Original HSG', ''
                ])
                ...
                writer.writerow([''] * 8)  # Empty separator row

    def save_summary_csv(self, subbasin_data: dict, results: dict,
                         output_dir: str):
        """Save summary CSV with both decimal and integer CN and area validation"""
        ...
        with open(summary_path, 'w', newline='', encoding='utf-8') as f:
            ...
            # Header row
            writer.writerow([
                'Subbasin_ID', 'Total_Area_Acres', 'Sum_CN_x_Area',
                'CN_Composite', 'CN_Integer'
            ])
            ...
            for subbasin_id, data in subbasin_data.items():
                if data['total_area'] > 0:
                    cn_comp = data['cn_area_sum'] / data['total_area']
                    cn_comp_rounded = round(cn_comp, 2)
                    cn_int = round(cn_comp)
                    area_acres = round(data['total_area'], 3)
                    cn_area_sum = round(data['cn_area_sum'], 3)
                    writer.writerow([
                        subbasin_id, area_acres, cn_area_sum,
                        cn_comp_rounded, cn_int
                    ])

            # Blank separator
            writer.writerow([])

            # Area validation footer
            ref_area = results.get('reference_area_acres', 0)
            int_area = results.get('intersection_area_acres', 0)
            diff = abs(ref_area - int_area)
            diff_pct = (diff / ref_area * 100) if ref_area > 0 else 0

            writer.writerow(['--- Area Validation ---'])
            writer.writerow(['Reference Subbasin Area (acres)', round(ref_area, 3)])
            writer.writerow(['Post-Intersection Area (acres)', round(int_area, 3)])
            writer.writerow(['Difference (acres)', round(diff, 3)])
            writer.writerow(['Difference (%)', round(diff_pct, 2)])
...
• CN values provided as both decimal and integer

📐 Area Validation:
• Reference area: {ref_area:.2f} acres
• Intersection area: {int_area:.2f} acres
• Difference: {diff:.2f} acres ({diff_pct:.2f}%)

📁 Output Files:
• subbasins_cn.shp — Shapefile with CN_Comp and CN_Int fields
• cn_calculations_detailed.csv — Detailed calculations
• cn_summary.csv — Summary table with area validation
