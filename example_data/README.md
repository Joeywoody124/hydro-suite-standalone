...
1. **Generate GIS Layers**: Run `create_sample_layers.py` in QGIS Python Console
2. **Load Lookup Tables**: Use CSV files when prompted by tools
3. **Follow Tutorials**: See `TUTORIALS.md` for step-by-step examples
...
| `create_sample_layers.py` | Script to generate sample GIS layers |
...
| `example_subbasins.csv` | Sample watershed data (tabular) | Reference |
| `example_channels.csv` | Sample channel designs (tabular) | Reference |
| `example_tc_flowpaths.csv` | Sample flow path data (tabular) | Reference |

### GIS Layers (Generated by Script)

Run `create_sample_layers.py` in QGIS to generate these GeoPackage layers:

| File | Type | Features | Description |
|------|------|----------|-------------|
| `sample_subbasins.gpkg` | Polygon | 5 | Drainage area boundaries with attributes |
| `sample_landuse.gpkg` | Polygon | 10 | Land use polygons matching lookup codes |
| `sample_soils.gpkg` | Polygon | 6 | SSURGO-style soils with HSG (A, B, C, D, A/D, B/D) |
| `sample_flowpaths.gpkg` | Line | 9 | TC flow path segments by type |
| `sample_channels.gpkg` | Line | 5 | Drainage channel centerlines |
| `sample_outlets.gpkg` | Point | 6 | Outlet/pour points |

**Default Output Location**: `Documents/HydroSuite_SampleData/`

**Coordinate System**: EPSG:2273 (SC State Plane South, feet)
